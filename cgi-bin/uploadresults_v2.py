    ...
    for i in mydict.keys():
        ...
        for j in mydict[i].keys():
            ...
            for k in mydict[i][j].keys():
                mydict2[i][j][k]={}
                for m in mydict[i][j][k].keys():
                    mydict2[i][j][k][m] = mydict[i][j][k][m][0]
    #print ('------',mydict2)
        
    print(form_template.format(options_map=json.dumps(opU),options_Site=options_Site,conditions=mydict2))
...
#!!!!
# get the lines
    dict_json['MVM_firmware_version'] =mydict[site][campaign][testID]['MVM Firmware'][0]
    dict_json['DAQ_software_version'] = mydict[site][campaign][testID]['DAQ version'][0]
    dict_json['comment'] = mydict[site][campaign][testID]['Comment'][0]
...
#
    conditions_dict = {}
    for key in mydict[site][campaign][testID].keys():
        conditions_dict[key]= mydict[site][campaign][testID][key][0]
...
#    print ('===================', dict_ids[site][(testID,campaign)][0])   
    ...
    if (mvmonly==False):
            res = insert_single_cell(mydict[site][campaign][testID]['simulator_filename'][1],mydict[site][campaign][testID]['simulator_filename'][2],filename_simulator_no_suffix, service,site, SAMPLE_SPREADSHEET_ID, VERB=VERB )
            if res == True:
...
# mvm
    res= insert_single_cell(mydict[site][campaign][testID]['MVM_filename'][1],mydict[site][campaign][testID]['MVM_filename'][2],file_mvm.filename, service, site,  SAMPLE_SPREADSHEET_ID ,VERB=VERB)
    if res == True:
...
##    if VERB==True:
#        print ("DB",db_s)
    mydict = dbToDict(db_s, VERB=VERB)
#    if VERB==True:
#        print ("DICT",mydict)
    ...
    if "submit" in form.keys():
        receiveAndSaveToGoogleSheet(mydict,service, SAMPLE_SPREADSHEET_ID,False)
    else:
        printForm(opU,opF,mydict)
